 ...
 * @author kingyang
 */
public class FaceSharedPreferences extends ContentProvider implements SharedPreferences {
    ...
    private void checkInitAuthority(Context context) {
        if (sAuthorityUrl == null) {
            synchronized (FaceSharedPreferences.this) {
                if (sAuthorityUrl == null) {
                    ...
                    if (packageInfos != null && packageInfos.providers != null) {
                        for (ProviderInfo providerInfo : packageInfos.providers) {
                            if (providerInfo.name.equals(FaceSharedPreferences.class.getName())) {
                    ...
                    if (sAuthoriry == null) {
                        throw new IllegalArgumentException("'AUTHORITY' initialize failed, Unable to find explicit provider class " + FaceSharedPreferences.class.getName() + "; have you declared this provider in your AndroidManifest.xml?");
                    }
     ...
     */
    public static FaceSharedPreferences getSharedPreferences(Context context, String name, int mode) {
        return new FaceSharedPreferences(context, name, mode);
    }

    public FaceSharedPreferences() {

    }

    private FaceSharedPreferences(Context context, String name, int mode) {
        mContext = context;
        mName = name;
        mMode = mode;
                            ...
                            if (mName.equals(name) && keysModified != null) {
                                ...
                                for (int i = keysModified.size() - 1; i >= 0; i--) {
                                    ...
                                    for (OnSharedPreferenceChangeListener listener : listeners) {
                                        if (listener != null) {
                                            listener.onSharedPreferenceChanged(FaceSharedPreferences.this, key);
                                        }
                                    }
                                }
                            }
    ...
    private String makeAction(String name) {
        return String.format("%1$s_%2$s", FaceSharedPreferences.class.getName(), name);
    }
