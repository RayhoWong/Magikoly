...
public class SharedUtil {
    ...
    public static Rect adjustRect(Bitmap bitmap, Rect faceRect) {
        Rect newRect = new Rect(faceRect);
        newRect.inset(-(int)(faceRect.width() * 0.25f), -(int)(faceRect.height() * 0.25f));
        if (newRect.left < 0) {
            newRect.left = 0;
        }
        if (newRect.right > bitmap.getWidth()) {
            newRect.right = bitmap.getWidth();
        }

        if (newRect.top < 0) {
            newRect.top = 0;
        }
        if (newRect.bottom > bitmap.getHeight()) {
            newRect.bottom = bitmap.getHeight();
        }
        return newRect;
    }
