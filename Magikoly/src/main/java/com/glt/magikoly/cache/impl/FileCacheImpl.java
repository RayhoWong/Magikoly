...
import com.glt.magikoly.cache.ICacheListener;
import com.glt.magikoly.cache.compress.ZipCompress;
import com.glt.magikoly.cache.encrypt.XORCrypt;
import com.glt.magikoly.cache.utils.CacheFileUtils;
...
public class FileCacheImpl extends BaseCacheImpl {
	...
	private void saveSync(String key, byte[] cacheData) throws Exception {
		...
		synchronized (lock) {
			...
			// 判断是否需要压缩
			if (mCompress != null) {
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_SAVE_COMPRESS_START, src,
						null);
				src = mCompress.compress(src);
				if (src == null) {
					throw new RuntimeException("Compress cache object  failed");
				}
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_SAVE_COMPRESS_FINISH, src,
						null);
			}
			...
			// 是否需要加密
			if (mEncrypt != null) {
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_SAVE_ENCRYPT_START, src,
						null);
				src = mEncrypt.encrypt(src);
				if (src == null) {
					throw new RuntimeException("Encrypt cache object  failed");
				}
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_SAVE_ENCRYPT_FINISH, src,
						null);
			}
	 ...
	 * <br>注意:
	 */
	private byte[] loadSync(String key) {
		if (!CacheFileUtils.isSDCardExist()) {
			throw new RuntimeException("SDCard is not exist");
		}
		Object lock = getLock(key);
...
//				e.printStackTrace();
			...
			// 解密数据
			if (mEncrypt != null) {
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_LOAD_DECRYPT_START, data,
						null);
				data = mEncrypt.decrypt(data);
				if (data == null) {
					throw new RuntimeException("Decrypt cache falied");
				}
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_LOAD_DECRYPT_FINISH, data,
						null);
			}
			...
			// 解压数据
			if (mCompress != null) {
				notifyListener(this, LISTENER_TYPE_PROGRESS, PROGRESS_LOAD_DECOMPRESS_START, data,
						null);
