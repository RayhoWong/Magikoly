...
public class AutoScrollViewPager extends ViewPager {
    ...
    private double                 mAutoScrollFactor            = 1.0;
    /** scroll factor for swipe scroll animation, default is 1.0 **/
    private double                 mSwipeScrollFactor           = 1.0;

    private Handler mHandler;
    private boolean                mIsAutoScroll                = false;
    private boolean                mIsStopByTouch               = false;
    private float                  mTouchX                      = 0f, mDownX = 0f;
	...
	public void startAutoScroll() {
		mIsAutoScroll = true;
		sendScrollMessage((long) (mInterval + mScroller.getDuration()
				/ mAutoScrollFactor * mSwipeScrollFactor));
	}
    ...
    public void setSwipeScrollDurationFactor(double scrollFactor) {
        mSwipeScrollFactor = scrollFactor;
    }
    ...
    public void setAutoScrollDurationFactor(double scrollFactor) {
        mAutoScrollFactor = scrollFactor;
    }
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		if (mSlideBorderMode == SLIDE_BORDER_MODE_TO_PARENT
			...
			if ((currentItem == 0 && mDownX <= mTouchX)
					|| (currentItem == pageCount - 1 && mDownX >= mTouchX)) {
				if (mSlideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
...
//		getParent().requestDisallowInterceptTouchEvent(true);
    ...
    private static class MyHandler extends Handler {
        ...
        private final WeakReference<AutoScrollViewPager> mAutoScrollViewPager;

        public MyHandler(AutoScrollViewPager autoScrollViewPager) {
            this.mAutoScrollViewPager = new WeakReference<AutoScrollViewPager>(autoScrollViewPager);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case SCROLL_WHAT:
                    AutoScrollViewPager pager = this.mAutoScrollViewPager.get();
                    if (pager != null) {
                        pager.mScroller.setScrollDurationFactor(pager.mAutoScrollFactor);
                        pager.scrollOnce();
                        pager.mScroller.setScrollDurationFactor(pager.mSwipeScrollFactor);
                        pager.sendScrollMessage(pager.mInterval + pager.mScroller.getDuration());
                    }
                default:
                    break;
            }
